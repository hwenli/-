 ...
 * 添加指定数量商品到购物车 
 */
function addToCart_num(goodsId, parentId , goods_number)
{
...
function addToCartResponse(result)
{
  ...
  {
	// document.getElementById('nogoods').innerHtml='';
	$('#nogoods').html('');
    ...
    else
    {
      ...
      {
        case '1' :
          if (confirm(result.message)) location.href = cart_url;//window.open(cart_url);
          break;
        case '2' :
          if (!confirm(result.message)) location.href = cart_url;//window.open(cart_url);
          break;
        case '3' :
          location.href = cart_url;//window.open(cart_url);
          break;
        default :
          break;
      }
...
function addToCartResponse2(result)
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
        case '1' :
          if (confirm(result.message))  alert('该商品已加入您的购物车！');//location.href = cart_url;//window.open(cart_url);
          break;
        case '2' :
          if (!confirm(result.message))  alert('该商品已加入您的购物车！');//location.href = cart_url;//window.open(cart_url);
          break;
        case '3' :
           alert('该商品已加入您的购物车！');//location.href = cart_url;//window.open(cart_url);
          break;
        default :
          break;
      }
    ...
    else
    {
      switch(result.confirm_type)
      {
        case '1' :
          ...
          if (confirm(result.message)) alert('该商品已加入您的购物车！');location.reload();
          break;
        case '2' :
          // if (!confirm(result.message)) location.href = cart_url;
          if (!confirm(result.message)) alert('该商品已加入您的购物车！');location.reload();
...
{
	jAlert('info', result.message, '温馨提示');
  // alert(result.message);
}
...
function setSuitShow(suitId)
{
    ...
    var suit2    = document.getElementById('suit2_'+suitId);
    var suit3    = document.getElementById('suit3_'+suitId);
    ...
    else
    {
        ...
        suit2.style.display='none';
        suit3.style.display='none';
    }
}
...
function openSpeDiv(message, goods_id, parent) 
{
  ...
  newDiv.style.border = "3px solid #999999";
  newDiv.style.padding = "5px";

  //生成层内内容
  newDiv.innerHTML = '<h4 style="font-size:14; margin:10px 0 10px 15px;">请选择商品属性</h4>';

  for (var spec = 0; spec < message.length; spec++)
  {
      newDiv.innerHTML += '<hr style="color: #EBEBED; height:1px;"><h6 style="text-align:left; background:#ffffff; margin-left:15px;margin-top:5px;margin-bottom:5px;">' +  message[spec]['name'] + '</h6>';
	  ...
	  function tab(id,hx,box,iClass,s,pr){
		  ...
		  function yy(){
			  for(var i=0;i<hxs.length;i++){
				  hxs[i].temp=i;
				  if(!s){// 如果不指定事件，则：
					  s="onmouseover"; // 使用默认事件，如果想点击切换，可以改成onclick
					  hxs[i][s]=function(){
						  go_to(this.temp);
					  }
				  }
